...
## 턴기반 전투 추가 (2026-07-16, 사용자 "게임이 되어야" 피드백)
- 문제: "처치" 클릭 즉시승리 → 스탯/성장이 무의미한 체크리스트였음.
- 해결: 퀘스트 클리어에 턴기반 전투를 넣어 성장 루프에 stakes 부여.
- 설계 결정:
  1. 전투는 매번 풀 HP로 시작(맵 상 HP/힐 관리 없음) → 접근성 유지하면서 전투 내 패배 가능.
  2. 몬스터 스탯은 `lib/battle.js`가 퀘스트 난이도×랭크배수로 결정적 생성.
  3. 행동: 공격/강공격(1.7배·30% 빗나감)/방어(피해 0.5배)/물약(45% 회복·2회).
  4. 데미지 = max(1, ATK - DEF*0.5) × 변동(0.85~1.15), 치명타 = 행운 기반 2배.
  5. 패배해도 페널티 없음(재도전) → 약하면 못 깨고 성장 필요.
  6. Dungeon 페이지 오버레이(모달)로 구현, 승리 시 기존 finishQuest 호출.
- 밸런스는 브라우저 플레이테스트로 튜닝.
